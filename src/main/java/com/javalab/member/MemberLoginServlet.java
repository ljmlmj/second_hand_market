...
public class MemberLoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, 
		...
		// 로그인 폼으로 이동		
		RequestDispatcher rds = request.getRequestDispatcher("/member/memberLogin.jsp");
	...
	protected void doPost(HttpServletRequest request, 
		...
		String pwd = "";
		String contextPath = request.getContextPath();
		String url = "/member/memberLogin.jsp";	//로그인 실패시 이동하는 페이지(다시 로그인 페이지)
		...
		MemberVo mb = new MemberVo(member_id, pwd);	// Dao에 인자로 보낼 객체 생성
		
		// 4 사용자 아이디/비밀번호 확인
		MemberVo member = memberDao.loginCheck(mb);
		...
		if(member == null) {
			// 4.2 로그인 오류 세팅
			request.setAttribute("loginErrMsg", "로그인 정보가 올바르지 않습니다.");	
			//out.print("false");
			RequestDispatcher rds = request.getRequestDispatcher(url);
			rds.forward(request, response);
			return;
