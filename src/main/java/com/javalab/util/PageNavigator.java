	...
	public String getPageNavigator(int totalCount, int listCount, int pagePerBlock,
		...
		if(totalCount > 0) {	//총 레코드 수가 하나라도 있어야 페이징
			...
			if(pageNum > 1){	// 쿼리스트링에서 & = &amp; 
				sb.append("<a href=\"").append("boardAll?pageNum=1&amp;searchText"+searchText);
				sb.append("\" title=\"<<\"><<</a>&nbsp;");
			}
			if (isPrev) {	//<< 가 있으면
				int goPrevPage = startPage - pagePerBlock;			
				sb.append("&nbsp;&nbsp;<a href=\"").append("boardAll?pageNum="+goPrevPage+"&amp;searchText="+searchText);
				sb.append("\" title=\"<\"><</a>&nbsp;&nbsp;");
			} else {
			...
			for (int i = startPage; i <= endPage; i++) {
				if (i == pageNum) {
					...
					sb.append("<a href=\"").append("boardAll?pageNum="+i+"&amp;searchText="+searchText);
			...
			if (isNext) {
				...
				sb.append("<a href=\"").append("boardAll?pageNum="+goNextPage+"&amp;searchText="+searchText);
				sb.append("\" title=\">\">></a>");
			} else {
				
			}
			if(totalNumOfPage > pageNum){
				sb.append("&nbsp;&nbsp;<a href=\"").append("boardAll?pageNum="+totalNumOfPage+"&amp;searchText="+searchText);
				sb.append("\" title=\">>\">>></a>");
			}
		}
